...
import { UIManager } from './ui.js?v=1778846971';
import { ItemDatabase, EconomyRules } from './db.js?v=1778846971';
import { GameSimulation } from './game_simulation.js?v=1778846971';
...
class Game {
    constructor() {
        ...
        this.gameState = this.sim.gameState;
        ...
        Object.defineProperty(this, 'bots', { get: () => this.gameState.bots });
        ...
        Object.defineProperty(this, 'gameTimer', { get: () => this.gameState.gameTimer, set: (v) => this.gameState.gameTimer = v });
    ...
    connectServer() {
        ...
        this.ws.onmessage = (e) => {
            ...
            if (msg.type === 'init') {
                ...
                this.player = this.gameState.players[this.localPlayerId];
                if (!wasDead && this.player && this.player.isDead) this.gameState.events.playerDied = true;
                if (!wasWon && this.player && this.player.won) this.gameState.events.playerWon = true;

                // Ensure inventory UI checks re-bind if raw data wiped instance
                if (this.gameState.events.inventoryDirty && this.ui) {
                    this.ui.refreshInventory();
                    this.gameState.events.inventoryDirty = false;
                }
            }
        };
        this.ws.onerror = (e) => {
    ...
    isPositionInGrass(x, y) {
        if (!this.gameState.grass) return false;
        for (let bush of this.gameState.grass) {
    ...
    update(dt) {
        ...
        if (this.input.mouseX !== undefined && this.input.mouseY !== undefined) {
        ...
        if (this.isMultiplayer) {
            ...
            this.sim.applyInput(this.localPlayerId, {
                move: { x: this.input.moveX || 0, y: this.input.moveY || 0 },
                shoot: this.input.isShooting,
                angle: this.player.rotation
            });
        ...
        if (this.gameState.events.playerDied) {
            document.getElementById('game-over').classList.remove('hidden');
            setTimeout(() => this.resetSession(), 3000);
            this.gameState.events.playerDied = false;
        }
        if (this.gameState.events.playerWon) {
            ...
            this.gameState.events.playerWon = false;
        }
        if (this.gameState.events.sessionReset) {
            ...
            this.gameState.events.sessionReset = false;
        }
        if (this.gameState.events.inventoryDirty) {
            if (this.ui) this.ui.refreshInventory();
            this.updateHUD();
            this.gameState.events.inventoryDirty = false;
        }
    ...
    dropItemToGround(itemId) {
        ...
        if (!this.gameState.groundItems) this.gameState.groundItems = [];
        this.gameState.groundItems.push(gItem);
    }

    populateLootGrid() {
        if (!this.gameState.groundItems) this.gameState.groundItems = [];
        const nearby = this.gameState.groundItems.filter(g => Math.hypot(g.x - this.player.x, g.y - this.player.y) < 150);
        ...
        for (let g of nearby) {
            ...
            if (slot) {
                this.gameState.groundItems = this.gameState.groundItems.filter(gi => gi.id !== g.id);
                const item = { 
    ...
    pickupClosestItem() {
        if (!this.gameState.groundItems || this.gameState.groundItems.length === 0) return;
        let closest = null;
        let minDist = 100; // pickup radius
        
        for (let g of this.gameState.groundItems) {
        ...
        if (closest) {
            ...
            if (slot) {
                this.gameState.groundItems = this.gameState.groundItems.filter(gi => gi.id !== closest.id);
                const item = { 
                    id: this.inventory.nextId++, 
    ...
    updateHUD() {
        ...
        if (labelsContainer) {
            if (this.gameState.groundItems && this.gameState.groundItems.length > 0) {
                const zoom = (this.player.cameraZoom && !isNaN(this.player.cameraZoom)) ? this.player.cameraZoom : 1.5;
                let i = 0;
                for (let g of this.gameState.groundItems) {
    ...
    renderFullMap() {
        ...
        if (this.gameState.grass) {
            ...
            for (let g of this.gameState.grass) {
                const gx = g.x * scaleX;
                const gy = g.y * scaleY;
                const gw = g.w * scaleX;
                const gh = g.h * scaleY;
