...
export class UIManager {
            ...
            if (e.key.toLowerCase() === 'tab') {
                ...
                if (!this.game.isInMenu) {
                    if (this.inventoryPanel.classList.contains('hidden') || this.inventoryPanel.style.display === 'none') {
                        ...
                        this.inventoryPanel.style.zIndex = '2000'; // ç¢ºä???canvas ??hud ä¸å±¤
                ...
                if (qItem) {
                    ...
                    if (inMenu && qItem.container !== 'stash') {
                        ...
                        if (slot) {
                            ...
                            this._showQuickEquipError("?åº«ç©ºé?ä¸è¶³ï¼ç¡æ³èª?ç§»??);
                        }
                        ...
                        if (!hasBackpack) {
                            this._showQuickEquipError("æ²æ?è£å??å?ï¼ç¡æ³å¿«?å¸è£?);
                            return;
                        }
                        ...
                        if (slot) {
                            ...
                            this._showQuickEquipError("?å?ç©ºé?ä¸è¶³");
                        }
                    }
                    return;
                }
    ...
    refreshShop() {
        ...
        const isWeaponTab = this.currentShopCategory === 'æ§æ¢°';

        if (isWeaponTab && curItemsContainer) {
            const weaponClasses = ['?¨é¨', ...new Set(activeCategoryItems.map(({ item }) => item.weaponClass || '?¶ä?'))];
            if (!this.currentWeaponSubCat || !weaponClasses.includes(this.currentWeaponSubCat)) this.currentWeaponSubCat = '?¨é¨';
            ...
            if (this.currentWeaponSubCat !== '?¨é¨') {
                activeCategoryItems = activeCategoryItems.filter(({ item }) => (item.weaponClass || '?¶ä?') === this.currentWeaponSubCat);
            }
        } else if (this.currentShopCategory === 'å­å?' && curItemsContainer) {
            const ammoClasses = ['?¨é¨', ...new Set(activeCategoryItems.map(({ item }) => item.ammoClass || '?ªç¥'))];
            if (!this.ammoClassSubCat || !ammoClasses.includes(this.ammoClassSubCat)) this.ammoClassSubCat = '?¨é¨';
            ...
            const classColors = {
                'å°å£å¾?: '#4a3a00', 'ä¸­å£å¾?: '#003a5a', '?¨å?å½?: '#3a1a5a',
                'å¤§å£å¾?: '#5a1a00', '?æ?å½?: '#1a4a1a', '???': '#5a3a00', '?¹æ?': '#5a0000'
            };
            ...
            if (this.ammoClassSubCat !== '?¨é¨') {
                activeCategoryItems = activeCategoryItems.filter(({ item }) => (item.ammoClass || '?ªç¥') === this.ammoClassSubCat);
            }
        } else {
            let subBarId = isOverlay ? 'overlay-weapon-subcat-bar' : 'lobby-weapon-subcat-bar';
            const subBar = document.getElementById(subBarId);
            if (subBar) subBar.remove();
            this.currentWeaponSubCat = '?¨é¨';
        }

        
        activeCategoryItems.sort((a, b) => {
            if (this.currentShopCategory === 'å­å?') {
        ...
        activeCategoryItems.forEach(({ key, item }) => {
                ...
                if (item.type === 'weapon') {
                    ...
                    imgHtml = `<div style="flex: 1; display:flex; justify-content:center; align-items:center; overflow:hidden; margin-top:5px; position:relative; min-height:60px;">
                        <img src="æ§æ¢°?ç?/${encodedName}.png" style="position:absolute; width: 100%; height: 100%; object-fit: fill; transform: scaleX(-1);" onerror="this.style.display='none'">
                    </div>`;
                }
                */
            ...
            if (dbItem.type === 'ammo') {
                ...
                contentHtml = `<div style="display:flex; flex-direction:column; justify-content:center; align-items:center; width:100%; height:100%; pointer-events:none; position:relative;">
                    ...
                    <div style="position:absolute; bottom:5px; padding:0 3px; background:rgba(0,0,0,0.7); border-radius:3px;">
                        <span style="color:${isBroken ? '#ff4444' : '#0f0'}; font-size:12px;">${isBroken ? 'å·²æ?å£? : item.durability.toFixed(1) + ' / ' + maxDur.toFixed(1)}</span>
                    </div>
                </div>`;
                ...
                contentHtml = `<img src="æ§æ¢°?ç?/${encodedName}.jpg"
                    alt="${item.typeId}"
                    ...
                    onerror="this.src='æ§æ¢°?ç?/${encodedName}.png'; this.onerror=function(){this.style.display='none'; this.parentElement.querySelector('.item-text-fallback').style.display='inline';}">
            ...
            div.addEventListener('dblclick', (e) => {
                ...
                if (isInRaid) {
                    ...
                    if (dbItem && dbItem.type === 'weapon' && dbItem.gridW * dbItem.gridH > 2) {
                        ...
                        if (!placed) {
                            this._showQuickEquipError('ä¸»æ­¦?¨æ?ä½è??å??å·²æ»¿ï??¡æ?å¿«éæ¾??);
                            return;
                        }
                        this.refreshInventory();
                        this.game.updateHUD();
                    ...
                    // ä½¿ç¨ autoEquipï¼ä??¥æ?çµåª?©ãå¸?stash?ç?è·¯å??é»??                    if (this.inv.autoEquipRaidSafe(item.id)) {
                        this.refreshInventory();
                        this.game.updateHUD();
                    } else {
                        this._showQuickEquipError('ç©ºé?ä¸è¶³?ç¡å°æ?æ¬ä?ï¼ç¡æ³å¿«?ç§»??);
                    }
                    return;
                }

                // === ä¸»é¸?®ï??æ?å®æ´?è¼¯ï¼å?è¨±å¸è£ï? ===
    ...
    showPurchaseToast(itemName) {
        ...
        toast.innerText = `å·²è³¼è²?${itemName}ï¼`;
        toast.style.cssText = `
    ...
    showItemStats(x, y, dbItem, itemInstance = null) {
        ...
        if (dbItem.type === 'weapon') {
            ...
            if (s.magSize) {
                ...
                if (itemInstance && itemInstance.hasUpgradedMag) {
                    upgradeText = ` <span style='color:#0f0'>(?´å®¹ ${s.upMagSize} ??</span>`;
                } else if (s.upMagSize) {
                    upgradeText = ` <span style='color:#aaa'>???¯å???${s.upMagSize} ??/span>`;
                }
                magInfo = `<div>å½å£å®¹é?: <span style="color:white">${currentMagCap} ??/span>${upgradeText}</div>`;
            }
            // Map ammoType to ammo category label
            const ammoClassLabels = {
                'å°å£å¾?:'å°å£å¾?(SMG/?æ?)', 'ä¸­å£å¾?:'ä¸­å£å¾?(AR/MG)',
                '?¨å?å½?:'?¨å?å½?(DMR/LMG)', 'å¤§å£å¾?:'å¤§å£å¾?(M250)',
                '?æ?å½?:'?æ?å½?(Sniper)', '???':'??? (Shotgun)',
                '?å·å½?:'?å·å½?(ASH-12)', 'ç´è?':'ç´è? (AWM)'
            };
            const ammoLabel = ammoClassLabels[dbItem.ammoType] || dbItem.ammoType || '-';
            const tierRestriction = dbItem.maxAmmoTier ? `ï¼æ?é«?${['','ç¶?,'??,'ç´?,'??][dbItem.maxAmmoTier] || dbItem.maxAmmoTier}å½ï?` : '';

            content = `
                <div>?·å®³: <span style="color:white">${damage}</span></div>
                <div>å°é? <span style="color:white">${s.fireRate ?? '-'}</span></div>
                <div>?´ç²?½å?: <span style="color:${s.armorPen ? '#00ffff' : 'white'}">${s.armorPen ? s.armorPen.toFixed(1) + 'x' : '1.0x'}</span></div>
                <div>?å??ç?: <span style="color:${(s.reloadMult||1) > 1 ? '#ff8800' : 'white'}">${s.reloadMult ? s.reloadMult.toFixed(1) + 'x' : '1.0x'}</span></div>
                <div>å¾åº§?§å¶: <span style="color:white">${s.recoil ?? '-'}</span></div>
                <div>ç²¾æ?åº? <span style="color:white">${s.accuracy ?? '-'}</span></div>
                <div>ç¯å?: <span style="color:white">${s.range ?? '-'}</span></div>
                <div>å­å??ºé? <span style="color:white">${s.velocity ?? '-'}</span></div>
                ${magInfo}
                <div style="color:#fa0; margin-top:6px;">å½è¥é¡å?: <span style="color:white">${ammoLabel}</span>${tierRestriction}</div>
                <hr style="border-color:#444; margin:8px 0;">
                <div>?é?: ${dbItem.weight} kg | å°ºå¯¸: ${dbItem.gridW}x${dbItem.gridH}</div>
            `;
        } else if (dbItem.type === 'armor' || dbItem.type === 'helmet') {
            ...
            const brokenStatus = itemInstance && itemInstance.isBroken ? `<div style="color:#ff4444; font-weight:bold; margin-top:5px;">??ï?å·²ç£¨?é?åº¦ï??¡æ?ä¿®å¾©</div>` : '';
            content = `
                <div>è­·ç²ç­ç?: <span style="color:white">Lv ${dbItem.level}</span></div>
                <div>?·å®³æ¸å?: <span style="color:white">${Math.round((dbItem.damageReduction || 0) * 100)}%</span></div>
                <div>?ä?åº? <span style="color:white">${durText}</span></div>
                ${itemInstance && itemInstance.originalMaxDurability ? `<div>?å?ä¸é?: ${itemInstance.originalMaxDurability}</div>` : ''}
                ${brokenStatus}
                <hr style="border-color:#444; margin:8px 0;">
                <div>?é?: ${dbItem.weight} kg | å°ºå¯¸: ${dbItem.gridW}x${dbItem.gridH}</div>
            `;
        } else if (dbItem.type === 'medical' || dbItem.type === 'medical-buff') {
            const uses = itemInstance ? `?©é? ${itemInstance.capacity ?? '-'} æ¬¡` : `?å¤?${dbItem.maxCapacity ?? '-'} æ¬¡`;
            content = `
                <div>é¡å?: <span style="color:white">${dbItem.healType || dbItem.effectType || '-'}</span></div>
                <div>ä½¿ç¨?é?: <span style="color:white">${((dbItem.useTime || 0)/1000).toFixed(1)}s</span></div>
                <div>${uses}</div>
                <hr style="border-color:#444; margin:8px 0;">
                <div>?é?: ${dbItem.weight} kg | å°ºå¯¸: ${dbItem.gridW}x${dbItem.gridH}</div>
            `;
            ...
            content = `
                <div>ä¿®ç?ç­ç?: <span style="color:white">Lv ${dbItem.level}</span></div>
                <div>å®¹é?: <span style="color:white">${itemInstance ? (itemInstance.capacity ?? dbItem.maxCapacity) : dbItem.maxCapacity}</span></div>
                <div>ä½¿ç¨?ç?: <span style="color:white">${dbItem.useRate} ??æ¬?/span></div>
                <hr style="border-color:#444; margin:8px 0;">
                <div>?é?: ${dbItem.weight} kg | å°ºå¯¸: ${dbItem.gridW}x${dbItem.gridH}</div>
            `;
        } else if (dbItem.type === 'ammo') {
            const tierNames = ['','ç¶ å?','?å?','ç´«å?','?è?'];
            const mods = dbItem.armorDamageMods || {};
            content = `
                <div>å½è¥?é?: <span style="color:#fa0">${dbItem.ammoClass || '-'}</span></div>
                <div>ç­ç?: <span style="color:white">${tierNames[dbItem.tier] || dbItem.tier || '-'}</span></div>
                <div>ç©¿ç²ç­ç?: <span style="color:white">Lv ${dbItem.penLevel || 0} ä»¥ä?è­·ç²?´æ¥ç©¿é?/span></div>
                <div style="margin-top:6px">è­·ç²?·å®³?ç?:</div>
                ${[1,2,3,4].map(l=>`<div style="padding-left:8px">Lv${l}: <span style="color:white">x${mods[l] ?? '-'}</span></div>`).join('')}
                ${dbItem.hpDamageMod !== undefined && dbItem.hpDamageMod !== 1.0 ? `<div>HP?·å®³?ç?: <span style="color:#f90">x${dbItem.hpDamageMod}</span></div>` : ''}
                ${dbItem.specialDesc ? `<div style="color:#f90; margin-top:4px">${dbItem.specialDesc}</div>` : ''}
                <hr style="border-color:#444; margin:8px 0;">
                <div>?é?: ${dbItem.weight} kg | ?¸é?: ${dbItem.maxCapacity}????/div>
            `;
        }
        ...
        if (dbItem.type === 'weapon' && itemInstance && !itemInstance.hasUpgradedMag && s.upMagPrice && s.upMagSize) {
            ...
            upBtn.innerText = `?ç?å½å£ (+${magIncrease}?? ?¹å·® $${priceDiff > 0 ? '+' + priceDiff : priceDiff})`;
        ...
        if (dbItem.type === 'weapon' && dbItem.ammoType) {
            ...
            buyAmmoBtn.innerText = `è³¼è²·å½è¥ ??${dbItem.ammoType}`;
            ...
            buyAmmoBtn.onclick = () => {
                ...
                if (ammoTabName) {
                    ...
                    const shopBtn = document.getElementById('shop-tab-btn') || [...document.querySelectorAll('button')].find(b => b.innerText.includes('?å?'));
    ...
    openRepairModal(kit, armor) {
        ...
        let content = `
            <div><strong>ä¿®ç?ç®±ï?</strong> ${kitName}</div>
            <div><strong>?®æ?è£å?ï¼?/strong> ${armorName}</div>
            <hr style="border-color:#333; margin:10px 0;">
            <div>?¶å??ä?ä¸é?ï¼?{result.currentMax.toFixed(1)}</div>
            <div>ç£¨æ??ç?ï¼?span style="color:#ff8800">${ratePct}%</span></div>
            <div style="font-size:16px; margin-top:10px;">
                ä¿®å¾©å¾é?è¨ä??ï?<span style="color:#00ffaa; font-weight:bold;">${result.predictedMax.toFixed(1)} / ${result.predictedMax.toFixed(1)}</span>
        ...
        if (result.isBroken) {
            content += `<div style="color:#ff4444; font-weight:bold; margin-top:10px; padding:10px; background:rgba(255,0,0,0.1); border-radius:5px;">
                è­¦å?ï¼ä¿®å¾©å??ä?ä¸é?å°ä??¼å?å§å¼ç? 20%ï¼è©²è£å?å°æ?å£ä??¡æ??ä¿®å¾©ï?
            </div>`;
        }
